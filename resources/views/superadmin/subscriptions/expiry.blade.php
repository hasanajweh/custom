...
@section('page-title', 'Subscription Expiry Tracker')
    ...
    <div class="space-y-6">
            ...
            <div class="glass rounded-lg p-6 border-l-4 border-red-500">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-red-400 mt-2">{{ $stats['expiring_week'] }}</p>
                    </div>
            ...
            <div class="glass rounded-lg p-6 border-l-4 border-yellow-500">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-yellow-400 mt-2">{{ $stats['expiring_month'] }}</p>
                    </div>
            ...
            <div class="glass rounded-lg p-6 border-l-4 border-orange-500">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-orange-400 mt-2">{{ $stats['expired'] }}</p>
                    </div>
            ...
            <div class="glass rounded-lg p-6 border-l-4 border-green-500">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-green-400 mt-2">{{ $stats['total_active'] }}</p>
                    </div>
        ...
        <div class="glass rounded-lg p-4">
            <div class="flex items-center space-x-4">
                <a href="{{ route('superadmin.subscriptions.expiry', ['filter' => 'expiring_week']) }}"
                   class="px-4 py-2 rounded-lg transition {{ $filter === 'expiring_week' ? 'bg-red-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600' }}">
                    This Week
                </a>
                <a href="{{ route('superadmin.subscriptions.expiry', ['filter' => 'expiring_soon']) }}"
                   class="px-4 py-2 rounded-lg transition {{ $filter === 'expiring_soon' ? 'bg-yellow-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600' }}">
                    Next 30 Days
                </a>
                <a href="{{ route('superadmin.subscriptions.expiry', ['filter' => 'expired']) }}"
                   class="px-4 py-2 rounded-lg transition {{ $filter === 'expired' ? 'bg-orange-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600' }}">
                    Expired
                </a>
                <a href="{{ route('superadmin.subscriptions.expiry', ['filter' => 'all']) }}"
                   class="px-4 py-2 rounded-lg transition {{ $filter === 'all' ? 'bg-indigo-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600' }}">
                    All Active
                </a>
            </div>
        </div>
        ...
        <div class="glass rounded-lg overflow-hidden">
            <table class="min-w-full">
                ...
                @forelse($subscriptions as $subscription)
                    ...
                    @endphp
                    <tr class="hover:bg-gray-800 transition-colors {{ $urgencyClass }}">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                ...
                                <div class="ml-4">
                                    <div class="text-sm font-medium text-white">{{ $subscription->school->name }}</div>
                                    <div class="text-sm text-gray-400">{{ $subscription->school->slug }}</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-white font-medium">{{ $subscription->plan->name }}</div>
                            <div class="text-sm text-gray-400">${{ number_format($subscription->plan->price_monthly / 100, 2) }}/mo</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-white">{{ $subscription->ends_at->format('M d, Y') }}</div>
                            <div class="text-xs text-gray-400">{{ $subscription->ends_at->format('h:i A') }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            @if($isExpired)
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                            <div class="flex items-center space-x-3">
                                ...
                                <a href="{{ route('superadmin.schools.edit', $subscription->school) }}"
        ...
        @if($subscriptions->hasPages())
            <div class="glass rounded-lg p-4">
                {{ $subscriptions->appends(['filter' => $filter])->links() }}
            </div>
        @endif
    </div>
@endsection
