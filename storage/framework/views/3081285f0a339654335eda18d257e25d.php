<?php $__env->startSection('title', __('messages.files.upload_file') . ' - ' . __('messages.app.name')); ?>
    ...
    <div class="max-w-2xl mx-auto">
        ...
        <?php if(session('upload_success')): ?>
            ...
            <div class="mb-6 bg-gradient-to-r from-emerald-50 via-green-50 to-teal-50 border-2 border-emerald-400 rounded-2xl p-6 sm:p-8 shadow-2xl animate-slideDown relative overflow-hidden">
                ...
                <div class="flex-1">
                                <p class="text-xl sm:text-2xl font-bold text-emerald-900 mb-1"><?php echo e($successTitleEn); ?></p>
                                <p class="text-xl sm:text-2xl font-bold text-emerald-900 mb-3"><?php echo e($successTitleAr); ?></p>
                                <p class="text-base sm:text-lg text-emerald-800 mb-1"><?php echo e($successMessageEn); ?></p>
                                <p class="text-base sm:text-lg text-emerald-800"><?php echo e($successMessageAr); ?></p>
                            </div>
                        </div>
                </div>
        ...
        <div class="text-center mb-6 sm:mb-8">
            ...
            <h1 class="text-2xl sm:text-3xl font-bold text-gray-900"><?php echo e(__('messages.files.upload_resource')); ?></h1>
            <p class="text-gray-600 text-sm sm:text-base"><?php echo e(__('messages.files.share_resource')); ?></p>
        </div>

        
        <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
            <form method="POST" action="<?php echo e(tenant_route('teacher.files.store', $school)); ?>" enctype="multipart/form-data" id="uploadForm">
                <?php echo csrf_field(); ?>

                <div class="p-5 sm:p-8 space-y-6 sm:space-y-8">
                    
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2"><?php echo e(__('messages.files.what_type_resource')); ?></label>
                        <div class="flex border border-gray-200 rounded-lg p-1 bg-gray-100 flex-col sm:flex-row gap-2 sm:gap-0">
                            <button id="generalBtn" type="button" class="form-toggle-btn active w-full sm:w-1/2 py-2"><?php echo e(__('messages.files.general_resource')); ?></button>
                            <button id="planBtn" type="button" class="form-toggle-btn w-full sm:w-1/2 py-2"><?php echo e(__('messages.files.lesson_plans')); ?></button>
                        </div>
                    </div>

                    
                    <div>
                        <label for="title" class="block text-sm font-medium text-gray-700 mb-1"><?php echo e(__('messages.files.resource_title')); ?> <span class="text-red-500">*</span></label>
                        <input type="text" id="title" name="title" value="<?php echo e(old('title')); ?>" required placeholder="<?php echo e(__('messages.placeholders.example_title')); ?>"
                    ...
                    <div id="generalResourceFields" class="space-y-6">
                        <?php if($missingAssignments): ?>
                            ...
                            <div class="p-4 bg-yellow-50 border border-yellow-200 rounded-lg text-sm text-yellow-800">
                                <?php echo e($assignmentMessage); ?>

                            </div>
                        <?php endif; ?>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2"><?php echo e(__('messages.files.content_type')); ?> <span class="text-red-500">*</span></label>
                            <div class="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3">
                                <label class="card-radio-label"><input type="radio" name="general_type" value="exam" class="sr-only peer" checked><span><?php echo e(__('messages.files.exam')); ?></span></label>
                                <label class="card-radio-label"><input type="radio" name="general_type" value="worksheet" class="sr-only peer"><span><?php echo e(__('messages.files.worksheet')); ?></span></label>
                                <label class="card-radio-label"><input type="radio" name="general_type" value="summary" class="sr-only peer"><span><?php echo e(__('messages.files.summary')); ?></span></label>
                            </div>
                        </div>

                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label for="subject_id" class="block text-sm font-medium text-gray-700 mb-1"><?php echo e(__('messages.users.subject')); ?> <span class="text-red-500">*</span></label>
                                <select id="subject_id" name="subject_id" class="w-full px-3 py-2 sm:px-4 bg-gray-50 border border-gray-300 rounded-lg text-sm sm:text-base focus:ring-blue-500 focus:border-blue-500" <?php if($subjects->isEmpty()): echo 'disabled'; endif; ?>>
                                    <option value=""><?php echo e(__('messages.files.select_subject')); ?></option>
                            ...
                            <div>
                                <label for="grade_id" class="block text-sm font-medium text-gray-700 mb-1"><?php echo e(__('messages.files.grade_level')); ?> <span class="text-red-500">*</span></label>
                                <select id="grade_id" name="grade_id" class="w-full px-3 py-2 sm:px-4 bg-gray-50 border border-gray-300 rounded-lg text-sm sm:text-base focus:ring-blue-500 focus:border-blue-500" <?php if($grades->isEmpty()): echo 'disabled'; endif; ?>>
                                    <option value=""><?php echo e(__('messages.files.select_grade')); ?></option>
                    ...
                    <div id="planFields" class="hidden space-y-6">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2"><?php echo e(__('messages.files.plan_duration')); ?> <span class="text-red-500">*</span></label>
                            <div class="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3">
                                <label class="card-radio-label"><input type="radio" name="plan_type" value="daily_plan" class="sr-only peer" checked><span><?php echo e(__('messages.plans.daily')); ?></span></label>
                                <label class="card-radio-label"><input type="radio" name="plan_type" value="weekly_plan" class="sr-only peer"><span><?php echo e(__('messages.plans.weekly')); ?></span></label>
                                <label class="card-radio-label"><input type="radio" name="plan_type" value="monthly_plan" class="sr-only peer"><span><?php echo e(__('messages.plans.monthly')); ?></span></label>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        <label for="file" class="block text-sm font-medium text-gray-700 mb-2"><?php echo e(__('messages.files.file_name')); ?> <span class="text-red-500">*</span></label>
                        ...
                        <label for="file" id="dropZone" class="flex justify-center items-center w-full h-28 sm:h-32 px-4 sm:px-6 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100 transition">
                            <div id="uploadPrompt" class="text-center text-sm sm:text-base">
                                ...
                                <p class="mt-1 text-gray-600"><span class="font-semibold text-blue-600"><?php echo e(__('messages.files.click_to_upload')); ?></span> or drag & drop</p>
                                <p class="text-xs text-gray-500 mt-1"><?php echo e(__('messages.files.unlimited_size')); ?></p>
                            </div>
                            <div id="fileInfo" class="hidden text-center text-sm sm:text-base">
                                ...
                                <button type="button" onclick="removeFile(event)" class="mt-2 text-red-600 text-xs sm:text-sm hover:underline"><?php echo e(__('messages.files.remove_file')); ?></button>
                            </div>
                        </label>
                    </div>
                ...
                <div class="px-5 sm:px-8 py-3 sm:py-4 bg-gray-50 border-t border-gray-200 flex flex-col sm:flex-row justify-end gap-3 sm:gap-4">
                    <a href="<?php echo e(tenant_route('teacher.files.index', $school)); ?>" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 w-full sm:w-auto text-center"><?php echo e(__('messages.actions.cancel')); ?></a>
                    <button type="submit" id="submitBtn" class="px-4 py-2 text-sm font-medium text-white bg-blue-600 border border-transparent rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed w-full sm:w-auto">
                        <span id="submitText"><?php echo e(__('messages.files.upload_file')); ?></span>
                        <span id="loadingSpinner" class="hidden">
                            <svg class="animate-spin h-5 w-5 text-white inline-block" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                            </svg>
                            <?php echo e(__('messages.status.uploading')); ?>
...
<script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const oldSubmissionType = '<?php echo e(old("submission_type")); ?>';
...
<?php echo $__env->make('layouts.school', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\eXtreme\Documents\custom\resources\views\teacher\files\create.blade.php ENDPATH**/ ?>