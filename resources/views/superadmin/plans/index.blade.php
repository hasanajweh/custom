                    ...
                    <tr class="hover:bg-gray-800 transition-colors">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div>
                                ...
                                <div class="text-sm text-gray-400">{{ $plan->slug }}</div>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-white">
                                <span class="font-medium">${{ number_format($plan->price_monthly / 100, 2) }}</span>/mo
                            </div>
                            <div class="text-sm text-gray-400">
                                ${{ number_format($plan->price_annually / 100, 2) }}/yr
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                ...
                                <span class="text-sm text-white">{{ $plan->storage_limit_in_gb }} GB</span>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <div class="max-w-xs">
                                @if($plan->features && is_array($plan->features))
                                    <ul class="text-sm text-gray-400 space-y-1">
                                        ...
                                        @if(count($plan->features) > 2)
                                            <li class="text-xs text-gray-500">+{{ count($plan->features) - 2 }} more</li>
                                        @endif
                                    </ul>
                                @else
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                            <div class="flex items-center space-x-2">
                                ...
                                <form method="POST" action="{{ route('superadmin.plans.destroy', $plan) }}" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
