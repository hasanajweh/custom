...
<div class="container mx-auto px-4 py-6 space-y-6">
    ...
    <div class="grid md:grid-cols-2 gap-6">
        <div class="bg-white rounded shadow p-4">
            <h2 class="text-lg font-semibold mb-3">{{ __('messages.main_admin.subjects_grades.create_title') }}</h2>
            <form method="post" action="{{ route('main-admin.subjects-grades.store', $network) }}" class="space-y-3">
                ...
                <div>
                    ...
                    <select name="type" class="w-full border rounded p-2" required>
                        <option value="subject">{{ __('messages.main_admin.users.subjects_label') }}</option>
                        <option value="grade">{{ __('messages.main_admin.users.grades_label') }}</option>
                    </select>
                </div>
                <div>
                ...
                <div>
                    <label class="text-sm text-gray-600 block mb-2">{{ __('messages.main_admin.subjects_grades.assign') }}</label>
                ...
                <div class="flex justify-end">
                    <button class="bg-indigo-600 text-white px-4 py-2 rounded">{{ __('messages.actions.save') }}</button>
                </div>
            </form>
        </div>
                    ...
                    <div class="border rounded p-3 space-y-2">
                        <form action="{{ route('main-admin.subjects-grades.update', [$network, 'subject', $subject->id]) }}" method="post" class="space-y-2">
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <button class="bg-gray-800 text-white px-3 py-1 rounded">{{ __('messages.actions.update') }}</button>
                            </div>
                        ...
                        <form action="{{ route('main-admin.subjects-grades.destroy', [$network, 'subject', $subject->id]) }}" method="post" onsubmit="return confirm('{{ __('messages.main_admin.common.confirm_archive') }}')">
                            @csrf
                            @method('delete')
                            <button class="text-red-600 text-sm">{{ __('messages.actions.archive') }}</button>
                        </form>
                    </div>
                @endforeach
            </div>
