...
<div class="container mx-auto px-4 py-6 space-y-4">
    <div class="space-y-2">
        <h1 class="text-2xl font-bold">{{ $network->name }} - {{ __('messages.main_admin.hierarchy.heading') }}</h1>
        <p class="text-gray-600">{{ __('messages.main_admin.hierarchy.subtitle') }}</p>
    </div>
        ...
        @foreach($branches as $branch)
            <div class="bg-white rounded shadow p-4">
                ...
                <div class="grid grid-cols-2 gap-2 text-sm">
                    ...
                    <div class="p-2 bg-gray-50 rounded">{{ __('messages.grades_label') }}: <strong>{{ $branch->grades_count }}</strong></div>
                </div>
            </div>
        @endforeach
    </div>
