...
@section('content')
    <div class="space-y-6">
        ...
        <div class="glass rounded-lg p-4">
            <form method="GET" action="{{ route('superadmin.schools.index') }}" class="flex items-center space-x-4">
                ...
                <select name="network" class="px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50">
                    ...
                    @foreach($networks as $network)
                        <option value="{{ $network->id }}" {{ request('network') == $network->id ? 'selected' : '' }}>{{ $network->name }}</option>
                    @endforeach
                </select>
            ...
            @forelse($schools as $school)
                <div class="glass rounded-lg p-6 card-hover">
                    <div class="flex items-start justify-between mb-4">
                        <div>
                            <h3 class="text-lg font-semibold text-white">{{ $school->name }}</h3>
                            <p class="text-sm text-gray-400">{{ $school->slug }}</p>
                            <p class="text-xs text-gray-500 mt-1">Network: {{ $school->network?->name ?? 'Unassigned' }}</p>
                        </div>
                        @if($school->is_active)
                    ...
                    <div class="space-y-3 mb-4">
                        <div class="flex items-center text-sm">
                            ...
                            <span class="text-gray-300">{{ $school->users_count ?? 0 }} users</span>
                        </div>
                        <div class="flex items-center text-sm">
                            <svg class="w-4 h-4 text-gray-400 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                            </svg>
                            <span class="text-gray-300">{{ $school->file_submissions_count ?? 0 }} files</span>
                        </div>
                        <div class="flex items-center text-sm">
                            <svg class="w-4 h-4 text-gray-400 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            @if($school->activeSubscription)
                                    {{ $school->activeSubscription->plan->name }}
                                @else
                                    No active plan
                                @endif
                        </span>
                    ...
                    <span class="text-xs text-gray-500">
                        ...
                        <div class="flex items-center space-x-2">
                            <a href="{{ route('superadmin.schools.users.index', $school) }}"
