        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
            ...
            <div id="generalSection" class="tab-content">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                    <form method="GET" action="{{ route('teacher.files.index', $school->slug) }}" id="generalFilterForm">
                        ...
                        <div class="flex gap-2">
                            ...
                            <a href="{{ route('teacher.files.index', $school->slug) }}" class="inline-flex items-center gap-1.5 px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-medium rounded-lg transition-colors">
                                ...
                                <tr class="hover:bg-gray-50 transition-colors duration-150">
                                    ...
                                    <td class="px-8 py-6">
                                        ...
                                        <div class="flex items-center justify-center space-x-2 rtl:space-x-reverse">
                                            ...
                                            {{-- Print Button --}}
                                            @if($canPreview)
                                                <button onclick="printFile('{{ route('teacher.files.preview', [$school->slug, $file->id]) }}', '{{ $file->title }}', '{{ Auth::user()->name }}', '{{ $file->created_at->format('M d, Y') }}')"
            ...
            <div id="plansSection" class="tab-content hidden">
                <!-- Filters -->
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                    <form method="GET" action="{{ route('teacher.files.index', $school->slug) }}" id="plansFilterForm">
                        ...
                        <div class="flex gap-2">
                            ...
                            <a href="{{ route('teacher.files.index', $school->slug) }}?tab=plans" class="inline-flex items-center gap-1.5 px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-medium rounded-lg transition-colors">
                                <i class="ri-refresh-line"></i>
                                <span>{{ __('messages.actions.reset') }}</span>
                            </a>
                        </div>
                                ...
                                <tr class="hover:bg-gray-50 transition-colors duration-150">
                                    ...
                                    <td class="px-8 py-6">
                                        ...
                                        <div class="flex items-center justify-center space-x-2 rtl:space-x-reverse">
                                            {{-- Preview Button --}}
                                            @if($canPreview)
                                                <a href="{{ route('teacher.files.preview', [$school->slug, $plan->id]) }}"
                                                   target="_blank"
                                                   class="p-2 text-blue-600 hover:text-blue-800 hover:bg-blue-50 rounded-lg transition-all duration-150"
                                                   title="{{ __('messages.files.preview_file') }}">
                                                    <i class="ri-eye-line text-lg"></i>
                                            ...
                                            {{-- Download Button - Always Active --}}
                                            <a href="{{ route('teacher.files.download', [$school->slug, $plan->id]) }}"
                                               class="p-2 text-green-600 hover:text-green-800 hover:bg-green-50 rounded-lg transition-all duration-150"
                                               title="{{ __('messages.files.download_file') }}">
                                                <i class="ri-download-line text-lg"></i>
                                            </a>
                                            ...
                                            {{-- Print Button --}}
                                            @if($canPreview)
                                                <button onclick="printFile('{{ route('teacher.files.preview', [$school->slug, $plan->id]) }}', '{{ $plan->title }}', '{{ Auth::user()->name }}', '{{ $plan->created_at->format('M d, Y') }}')"
                                                        class="p-2 text-purple-600 hover:text-purple-800 hover:bg-purple-50 rounded-lg transition-all duration-150"
                                                        title="{{ __('messages.files.print_file') }}">
                                                    <i class="ri-printer-line text-lg"></i>
                                                </button>
