...
@section('title', __('messages.dashboard.teacher_dashboard') . ' - ' . __('messages.app.name'))
            ...
            <div class="relative z-10">
                <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between">
                    <div class="mb-6 lg:mb-0">
                        ...
                        <p class="text-indigo-100 text-lg">{{ __('messages.dashboard.ready_share_knowledge') }}</p>
                    </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div class="bg-white rounded-xl p-6 border border-gray-100 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-gray-600 mb-1">{{ __('messages.dashboard.my_uploads') }}</p>
                        <p class="text-3xl font-bold text-gray-900">{{ $totalUploads }}</p>
                        <p class="text-xs text-gray-500 mt-2">{{ __('messages.dashboard.total_files_shared') }}</p>
                    </div>
            ...
            <div class="bg-white rounded-xl p-6 border border-gray-100 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-gray-900">{{ \App\Models\FileSubmission::where('user_id', Auth::id())->where('school_id', $school->id)->whereBetween('created_at', [now()->startOfWeek(), now()->endOfWeek()])->count() }}</p>
                        <p class="text-xs text-gray-500 mt-2">{{ __('messages.dashboard.files_uploaded') }}</p>
                    </div>
            ...
            <div class="bg-white rounded-xl p-6 border border-gray-100 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-gray-600 mb-1">{{ __('messages.dashboard.recent_activity') }}</p>
                        <p class="text-3xl font-bold text-gray-900">{{ $recentFiles->count() }}</p>
                        <p class="text-xs text-gray-500 mt-2">{{ __('messages.dashboard.files_this_week') }}</p>
                    </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        @forelse($recentFiles as $file)
                            <div class="flex items-center justify-between p-4 hover:bg-gray-50 rounded-xl transition-all group">
                                <div class="flex items-center space-x-4">
                                    ...
                                    <div>
                                        <p class="text-sm font-semibold text-gray-900">{{ Str::limit($file->title, 40) }}</p>
            ...
            <div class="bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 rounded-xl p-6 border border-indigo-100">
                <h3 class="text-lg font-semibold text-gray-900 mb-6 flex items-center">
                    ...
                    {{ __('messages.dashboard.quick_tips') }}
                </h3>
                <div class="space-y-4">
                    <div class="bg-white/70 backdrop-blur rounded-xl p-4 border border-white/50 hover:shadow-md transition-all">
                        <div class="flex items-start space-x-3">
                            ...
                            <div>
                                <p class="text-sm font-semibold text-gray-900">{{ __('messages.files.clear_titles') }}</p>
                                <p class="text-xs text-gray-600 mt-1">{{ __('messages.files.use_descriptive_titles') }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white/70 backdrop-blur rounded-xl p-4 border border-white/50 hover:shadow-md transition-all">
                        <div class="flex items-start space-x-3">
                            ...
                            <div>
                                <p class="text-sm font-semibold text-gray-900">{{ __('messages.files.file_formats') }}</p>
                                <p class="text-xs text-gray-600 mt-1">{{ __('messages.files.pdf_word_excel') }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white/70 backdrop-blur rounded-xl p-4 border border-white/50 hover:shadow-md transition-all">
                        <div class="flex items-start space-x-3">
                            ...
                            <div>
                                <p class="text-sm font-semibold text-gray-900">{{ __('messages.files.regular_uploads') }}</p>
                                <p class="text-xs text-gray-600 mt-1">{{ __('messages.files.share_weekly') }}</p>
                            </div>
                        </div>
                    </div>

                    <div class="bg-gradient-to-r from-purple-600 to-indigo-600 rounded-xl p-4 text-white mt-6">
                        <p class="text-sm font-semibold mb-2">{{ __('messages.dashboard.pro_tip') }}</p>
                        <p class="text-xs">{{ __('messages.files.upload_best_materials') }}</p>
                    </div>
                </div>
            </div>
        </div>
